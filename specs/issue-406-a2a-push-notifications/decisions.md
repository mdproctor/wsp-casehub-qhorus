...
## D3: Delivery model — ChannelBackend with AT_LEAST_ONCE delivery

**Choice:** `PushNotificationBackend` implements `ChannelBackend` with `DeliveryGuarantee.AT_LEAST_ONCE`. The delivery pump calls `post()` for every message on registered channels. `post()` filters by correlationId — messages whose correlationId matches an active push config are pushed to the configured URL as a `TaskStatusUpdateEvent`; non-matching messages are a no-op and the cursor advances. The delivery pump provides cursor-based retry (via `DeliveryBatchExecutor`), health tracking with circuit breaker (via `DeliveryService.recordFailure()`/`isUnhealthy()`), 30s scheduled reconciliation (via `reconcileAll()`), and Micrometer metrics — all without duplication.
**Alternatives:**
- JPA-backed outbox with `@Scheduled` retry — persistent outbox table with exponential backoff (5s, 30s, 2m, 10m, 1h). Reliable but duplicates the delivery pump's retry, health, reconciliation, and metrics infrastructure. Creates a second delivery mechanism within qhorus.
- Fire-and-forget (virtual threads) — simple but unreliable. Issue #406 explicitly requests "retry + health tracking."
**Rationale:** `A2AOutboundBackend` is the direct precedent: it implements `ChannelBackend` with `AT_LEAST_ONCE`, selectively filtering in `post()` based on target binding resolution — messages without a matching `ExternalAgentBinding` are silently passed over, the cursor advances. A `PushNotificationBackend` follows the identical pattern, filtering by correlationId against the push config store. Per the A2A spec, each task has at most one push config, so `post()` delivers to at most one target per message — identical failure granularity to `A2AOutboundBackend`. On failure, `post()` throws, the cursor stalls, and the pump retries via its standard batch/reconciliation cycle. ADR-0018's prohibition applies to the platform's `DeliveryAttempt`/`EngagementEvent` SPI (attempt-shaped, per-recipient), not to qhorus's own cursor-based delivery pump.
**Trade-offs:** Fixed-interval retry (30s reconciliation) instead of per-event exponential backoff. Per-backend health tracking rather than per-push-target. Both are acceptable: 30s retry is sufficient for HTTP push, and per-backend health matches the existing `A2AOutboundBackend` contract. The pump can be extended with backoff support if needed.
**Sources:** `A2AOutboundBackend.java` (AT_LEAST_ONCE + selective filtering precedent), `DeliveryService.java` (health tracking, reconciliation), `DeliveryBatchExecutor.java` (cursor-based batch delivery), `ChannelGateway.fanOut()` (tracked backend skip), ADR-0018 (platform SPI mismatch — does not apply to qhorus delivery pump)
**Exploration:** quick
**Status:** revised — R1-02/R1-03: adopted ChannelBackend pattern, eliminated outbox. A2AOutboundBackend precedent confirmed: selective filtering in post() with AT_LEAST_ONCE delivery reuses the pump without duplication.

## D4: Push config storage — PushNotificationConfig entity with cleanup lifecycle

**Choice:** New JPA entity `PushNotificationConfigEntity` in the a2a-push-notification module, stored in the qhorus named PU. Fields: id, taskId (correlationId), channelId, url, token, authScheme, authCredentialsRef, tenancyId, createdAt. Cleanup lifecycle: (1) When the push backend delivers a terminal-state event (DONE → "completed", FAILURE → "failed", DECLINE → "canceled"), it deletes the push config after successful delivery. (2) A `@Scheduled` TTL cleanup job removes push configs older than a configurable threshold (e.g., 24h) to handle abandoned configs where the task never reached terminal state. (3) Client-initiated deletion via `tasks/pushNotificationConfig/delete` JSON-RPC method per the A2A spec.
**Alternatives:**
...
**Exploration:** quick
**Status:** revised — R1-06: added three-tier cleanup lifecycle (terminal delivery, TTL, client delete)
